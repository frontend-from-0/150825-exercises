...
function validateCard() {
  if (cardPattern.test(cardnumberInput.value)) {
    ...
    cardnumberError.innerText = 'Please enter correct card number (e.g. 1234 1234 1234 1234)';
    formCorrect = false;
  }
}
...
function validateExpDate() {
  if (expDatePattern.test(expDateInput.value)) {
      ...
      expDateError.innerText = 'Please enter correct expiration date (e.g. 01/26)';
      formCorrect = false;
    }
}
...
function validateCVV() {
   if (cvvPattern.test(cvvInput.value)) {
      ...
      cvvError.innerText = 'Please enter correct CVV number (e.g. 123)';
      formCorrect = false;
    }
}
...
formElement.addEventListener('submit', function (event) {
  ...
  if (formCorrect) {
    ...
    successDiv.innerHTML = `<p> Thank you ${firstNameInput.value}, your order has been successfully received!</p>`;
