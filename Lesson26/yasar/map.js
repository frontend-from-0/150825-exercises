...
const studentDetails = studentsExercise5.map(student => `${student.name} ${student.grade}`);
...
const bookDetails = booksExercise9.map(book =>`${book.title} by ${book.author}`);
...
const pricesDetails = pricesExercise10.map(price =>`$${price}`);
