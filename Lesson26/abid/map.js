...
const wordsofSentences = sentencesExercise7.map(sentence => sentence.split(' '));
console.log(wordsofSentences);
...
const titleBookAuthors = booksExercise9.map(book => `${book.title} - ${book.author}`);
console.log(titleBookAuthors);
...
const symbolOfPrice = pricesExercise10.map(price=> `$${price}`);
console.log(symbolOfPrice);