...
const studentGrade = studentsExercise5.map((student) => {
...
const numberToString = numbersExercise6.map((number) => String(number));
const numberToString2 = numbersExercise6.map((num) => num.toString());
const numberToString3 = numbersExercise6.map(String);

console.log("6:" + numberToString);
...
const sentenceToWordsArray = sentencesExercise7.map((sentence) =>
  sentence.split(" "),
);

console.log("7:" + sentenceToWordsArray);
...
const priceWithCurrency = pricesExercise10.map((price) => {
  return `$${price}`;
});
