...
const studentSummaries = studentsExercise5.map(
...
const wordsPerSentence = sentencesExercise7.map((sentence) =>
  sentence.split(" "),
);
...
const bookList = booksExercise9.map(({ title, author }) => ({ title, author }));
...
const formattedPrices = pricesExercise10.map((price) => "$" + price);
