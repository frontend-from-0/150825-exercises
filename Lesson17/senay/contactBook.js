...
console.log("--1. Partial Name Search:----------------");
...
///////////////////////////////////////////////////////
console.log("--OPTIONAL ENHANCEMENTS:----------------");
console.log("--3. Search by multiple fields:---------");
/*3. Search by multiple fields:
   - e.g., find a contact by phone number or email.
*/
function searchContact(name, phone, email) {
  ...
  if (searchBy.length === 0) {
    console.log("Please provide at least one search criteria.");
    return;
  }
  ...
  for (const contact of contacts) {
    ...
    if (matchCount === searchBy.length) {
      count++;

      console.log(
        `The contact found by ${searchBy.join(" and ")} where ${Object.values(
          searchObject
        ).join(" and ")}`
      );
      console.log(
        `The contact is: ${contact.name} | Tel: ${contact.phone} | Email: ${contact.email}`
      );
    }
  }
