...
function fetchUsers() {
  ...
  fetch('https://dummyjson.com/users')
    .then((response) => {
      if (!response.ok) {
        throw Error(`Failed fetching all users: ${response.status} ${response.statusText}`);
      }
      return response.json();
    })
    .then((data) => {
      data.users.forEach((user) => createUserCard(user));
      showStatus('Users loaded successfully', false);
    })
    .catch((error) => {
      console.error(error);
...
function deleteUser(userId, cardElement) { 
  fetch(`https://dummyjson.com/users/${userId}`, { 
    ...
    .then((res) => { 
      if (!res.ok) { 
        throw Error(`Failed deleting user ${userId}: ${res.status} ${res.statusText}`);
      }
      return res.json();
    })
    .then((data) => {
      showStatus(`User ${data.firstName} ${data.lastName} was deleted.`, false);
      cardElement.remove(); 
    })
    .catch((error) => {
      console.error(error);
